...
Activation functions are often selected based on the range of their output values, in cases where this is unimportant it is common to use the Rectified Linear Unit (ReLU) function as it provides strong gradients, which aids in the prevention of vanishing gradients in deep network models.

\subsection{Sigmoid}
The sigmoid function, expressed by equation (\ref{eq:sigmoid}) is a logistic function 
...
\begin{figure}[h!]
    ...
    \end{subfigure}
    \caption{ReLU Activation Functions}\label{fig:relus}
\end{figure}
